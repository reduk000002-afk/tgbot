...
# ========== КОНФИГУРАЦИЯ ==========
# ВСЕ КЛЮЧИ ТОЛЬКО ИЗ ПЕРЕМЕННЫХ ОКРУЖЕНИЯ RAILWAY
TOKEN = os.getenv("BOT_TOKEN")
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN")
GITHUB_REPO_OWNER = os.getenv("GITHUB_REPO_OWNER")
GITHUB_REPO_NAME = os.getenv("GITHUB_REPO_NAME")

# Значения по умолчанию (на случай если переменные не настроены)
if not TOKEN:
    logger.error("❌ BOT_TOKEN не настроен в Railway Variables!")
    TOKEN = ""  # Без токена бот не запустится

if not GITHUB_TOKEN:
    logger.warning("⚠️ GITHUB_TOKEN не настроен. Данные будут сохраняться только локально")

if not GITHUB_REPO_OWNER:
    GITHUB_REPO_OWNER = "reduk000002-afk"  # Значение по умолчанию

if not GITHUB_REPO_NAME:
    GITHUB_REPO_NAME = "tgbot"  # Значение по умолчанию
...
# Твой Telegram ID (админ) - из переменной окружения или по умолчанию
...
print(f"✅ GITHUB_TOKEN: {'Настроен' if GITHUB_TOKEN else '⚠️ НЕТ (локальный режим)'}")
...
async def save_user(telegram_id: str, login: str, name: str) -> bool:
    ...
    if telegram_id not in _user_nicks:
        _user_nicks[telegram_id] = []
    
    # Пробуем сохранить на GitHub если есть токен
    if not GITHUB_TOKEN:
        logger.warning("⚠️ GITHUB_TOKEN не настроен, сохраняю только локально")
        return True
        ...
        async with aiohttp.ClientSession() as session:
            async with session.put(url, headers=headers, json=payload) as response:
                if response.status in [200, 201]:
                    ...
                    logger.error(f"❌ Ошибка сохранения пользователя на GitHub: {response.status}")
                    logger.info("⚠️ Сохраняю локально")
                    return True  # Все равно успешно сохранили локально
...
async def save_nick(nick: str, manager_id: str, manager_name: str) -> bool:
    ...
    # Сначала загружаем текущие ники с GitHub (если есть токен)
                    ...
                    if response.status == 200:
                        ...
                        for n, info in nicks_data.get("nicks", {}).items():
                            ...
                            if user_id not in _user_nicks:
                                _user_nicks[user_id] = []
                            if n not in _user_nicks[user_id]:
                                _user_nicks[user_id].append(n)
                    else:
                        logger.warning(f"📝 Файл ников не найден, создаем новый")
        except Exception as e:
            logger.error(f"⚠️ Ошибка загрузки ников с GitHub: {e}")
            logger.info("Работаем с локальными данными")
    else:
        logger.warning("⚠️ GitHub токен не настроен, работаем локально")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.put(url, headers=headers, json=payload) as response:
                    if response.status in [200, 201]:
                        ...
                        error_text = await response.text()
                        logger.error(f"❌ Ошибка сохранения ника на GitHub: {response.status}")
                        logger.info(f"✅ Ник '{nick}' сохранен локально")
                        return True  # Все равно сохранили локально
        except Exception as e:
...
async def get_all_nicks() -> List[Dict]:
    ...
    # Загружаем с GitHub при первом обращении (если есть токен)
    if GITHUB_TOKEN and not _local_nicks:
        try:
            ...
            logger.error(f"⚠️ Ошибка загрузки ников: {e}")
...
def main():
    ...
    print(f"👥 Доступно пользователей: {len(VALID_USERS)}")
    print(f"🔐 Админ: test / 12345")
    print(f"🌐 GitHub: {'✅ Настроен' if GITHUB_TOKEN else '⚠️ Локальный режим'}")
